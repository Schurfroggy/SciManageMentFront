...
setTimeout(() => {
  // 从后端api获取数据
  $api.get('/secretary/getAll').then(res => {
    fresh(res)
  })
}, 500)
function addClue() {
...
function handleDelete(row) {
  ElMessageBox({
    ...
    callback: (action) => {
      if (action === 'confirm') {
        $api.delete_('/secretary/' + row.secretaryID).then(() => {
...
function getList(val) {
  if (val) {
    ...
    $api.get('/secretary/searchByName?name=' + val.secretaryName).then(res => {
      fresh(res)
    })
  }
}
