...
setTimeout(() => {
  // 从后端api获取数据
  $api.get('/client/getAll').then(res => {
    fresh(res)
  })
}, 500)
...
function handleView(row) {
  $api.get('/client/' + row.clientID).then(res => {
...
function handleDelete(row) {
  ElMessageBox({
    ...
    callback: (action) => {
      if (action === 'confirm') {
        $api.delete_('/client/' + row.clientID).then(() => {
...
function getList(val) {
  if (val) {
    ...
    $api.get('/client/searchByName?name=' + val.clientName).then(res => {
      fresh(res)
    })
  }
}
