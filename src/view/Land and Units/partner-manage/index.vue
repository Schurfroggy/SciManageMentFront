...
setTimeout(() => {
  // 从后端api获取数据
  $api.get('/partner/getAll').then(res => {
    fresh(res)
  })
}, 500)
...
function handleView(row) {
  $api.get('/partner/' + row.partnerID).then(res => {
...
function handleDelete(row) {
  ElMessageBox({
    ...
    callback: (action) => {
      if (action === 'confirm') {
        $api.delete_('/partner/' + row.partnerID).then(() => {
...
function getList(val) {
  if (val) {
    ...
    $api.get('/partner/searchByName?name=' + val.partnerName).then(res => {
      fresh(res)
    })
  }
}
...
function fresh(res) {
  ...
  for (let i = 0; i < res.total; i++) {
    tableData.push({
      partnerID: res.data[i].partner_id,
